...
def start_message(message):
    bot.send_message(message.chat.id,'Привет, чтобы получить список команд и краткую инструкцию по кнопкам,'
                                     ' напиши: /help',reply_markup=keyboard)
...
def get_help(message):
    ...
    bot.send_message(message.chat.id,'Список команд: \n'
                                     ...
                                     'Описание кнопок и инструкция \n\n'
                                     ' Привет' + ' - Поздороваться\n'
                                     ' Пока' + ' - Попрощаться\n\n'
                                     ' Для того, чтобы отслеживать изменение товаров, сначала необходимо внести их бд.' \
                                      + ' Для внесения воспользуетесь кнопкой "Добавить сайты". Товары нужно ' \
                                     + 'добавлять в виде "Название Кампании:Ссылка на товар"(без ковычек). \n'
                                     ' Для разовой проверки товаров воспользуйтесь командой /check. ' \
                                     + 'Чтобы удалить товары из отслеживания воспользуйтесь кнопкой "Удалить сайты."\n'
                                     'Удаление производится путем указания имени кампании. \n'
                                     'Для начала отслеживание нажмите соответствующую кнопку. Обновление ' \
                                     + 'произодиться раз в 4 часа. Для остановки выберите соответствующую кнопку\n'
...
def send_text(message):
    if message.text.lower() == 'привет':
        ...
        bot.send_sticker(message.chat.id, 'CAACAgIAAxkBAAEBMepfNUJNrJQw_aqf5Tilh4wrlPmVyAACCgADFvHqEpRNGYEZoiEhGgQ')
...
def make_thread(message):
    # create timer to rerun this method in 4 hours (in seconds)
    global my_timer
